...
if (selRef.length != 1) {
  alert("Select one and only one text object. Cannot export.");
} else {
  main();
}
...
function handlePathRef(pathRef, allPoints, paths) {
  ...
  if (pathRef.typename == "GroupItem") {
    ...
    return [{ points: allPoints, paths: paths }];
  } else {
    try {
...
function main() {
  ...
  for (i=0; i<selRef.length; i++) {
    nextTask("Processing shape " + i + " of " + selRef.length);

    // Duplicate the shape so we can do point expansion without modifying the users' shape
	
    //var newRef = selRef[i].duplicate(selRef[i].parent, ElementPlacement.PLACEATEND);
    var newRef = selRef[i].duplicate(selRef[i].parent, ElementPlacement.PLACEATEND).createOutline();
  ...
  shapes.forEach(function(shape){
    shape.points = shape.points.map(function(pt) {
        return {x: pt.x-bounds.min.x, y: pt.y-bounds.min.y};
    });
  });
  ...
  shapes.forEach(function(shape, i) {
    strScadCode += "module " + options.name + "_shape" + i + "() {\n";
    if (shape.paths) {
      strScadCode += "\tpolygon(points=" + options.name + "_shapePoints[" + i + "], paths=" + options.name + "_shapePaths[" + i + "]);\n";
    } else {
     strScadCode += "\tpolygon(points=" + options.name + "_shapePoints[" + i + "]);\n";
    }
    strScadCode += "}\n";
  });
...
function showOptionsDialog() {
  var w = new Window ("dialog", "Generate OpenSCAD Stamp");
  ...
  infoPanel.text = "Stamp Name";
  var infoName = infoPanel.add("edittext", undefined, selRef[0].contents.toLowerCase().replace(/[^a-z0-9\_]+/g, "_"));
  ...
  dividePanel.text = "Letters resolution";
  dividePanel.orientation = "row";

  var divisionLengthSlider = dividePanel.add ("slider", undefined, 0.5, .1, 100);
  var divisionLengthText = dividePanel.add ('edittext {text: "0.5", characters: 5, justify: "center"}');
